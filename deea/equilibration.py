...
from .sse import get_sse_series_init_seq, get_sse_series_window, sse
...
def get_sse_series(
    ...
    method: str = "lugsail",
    n_pow: float = 1 / 3,
    lugsail_max_discard_frac: float = 0.95,
) -> _Tuple[_np.ndarray, _np.ndarray]:
    """
    Return the a series of estimates of the squared standard error (SSE)
    from a (set of) timeseries. The SSE is obtained by estimating the SSE at
    at each time point, discarding all samples before the time point. The index
    of the time point with the minimum SSE is taken to be the point of
    equilibration.
    ...
    data : np.ndarray
        The time series data. This should have shape
        (n_runs, n_samples) or (n_samples,). If the method
        is 'lugsail' or 'intra', the data may have only one run, but
        otherwise there must be at least two runs.
    ...
    method : str, optional
        The method to use to estimate the limiting variance. This can be
        'lugsail', 'intra', or 'inter'. The default is 'lugsail'. 'lugsail'
        uses the lugsail replicated batch means variance estimate, 'intra'
        uses the mean intra-run variance estimate, and 'inter' uses the
        inter-run variance estimate.
    ...
    n_pow : float, optional
        ...
        is 'intra' or 'inter', this parameter is ignored.

    lugsail_max_discard_frac : float, optional
        The maximum fraction of samples to discard when using the lugsail
        method. The default is 0.95. This is required to avoid errors from
        the lugail method when the smaller batch size becomes too small.
    """
    # Check that method is valid.
    ...
    # Check that times match the number of samples.
    if n_samples != len(times):
        raise InvalidInputError(
            "Times must have the same length as the number of samples."
        )

    # If using the lugsail method, limit the maximum number of samples
    # that we discard to ensure that we don't get errors due to the smaller
    # batch size becoming too small.
    keep_frac = lugsail_max_discard_frac  # if method == "lugsail" else 1.0
    final_idx = round(n_samples * keep_frac)
    n_ignore = n_samples - final_idx
    # Make sure that this corresponds to at least 5 samples.
    if n_ignore < 5 and method == "lugsail":
        raise AnalysisError("The time series is too short.")

    # Compute the limiting variance at each time point.
    lim_var_vals = _np.zeros(n_samples - n_ignore)
    time_vals = _np.zeros(n_samples - n_ignore)

    for i in range(n_samples - n_ignore):
        # Get the truncated data.
        truncated_data = data[:, i:]
        # Compute the ESS.
        lim_var_vals[i] = sse(truncated_data, method=method, n_pow=n_pow)
        # Get the time.
        time_vals[i] = times[i]

    return lim_var_vals, time_vals


def detect_equilibration_min_sse(
    data: _np.ndarray,
    times: _Optional[_np.ndarray] = None,
    method: str = "lugsail",
    max_min_lim_var: float = float("inf"),
    n_pow: float = 1 / 3,
    lugsail_max_discard_frac: float = 0.95,
    plot: bool = False,
    plot_name: _Union[str, Path] = "equilibration_sse.png",
    time_units: str = "ns",
    data_y_label: str = r"$\Delta G$ / kcal mol$^{-1}$",
) -> _Tuple[int, float, float]:
    r"""
    Detect the equilibration time of a time series by finding the minimum
    squared standard error (SSE) of the time series. This is done by
    computing the SSE at each time point, discarding all samples before
    the time point. The index of the time point with the minimum SSE is taken
    ...
    method : str, optional
        The method to use to estimate the limiting SSE. This can be
        'lugsail', 'intra', or 'inter'. The default is 'lugsail'. 'lugsail'
        uses the lugsail replicated batch means limiting variance estimate,
        'intra' uses the limiting mean intra-run limiting variance estimate,
        and 'inter' uses the inter-run variance estimate.

    max_min_lim_var : float, optional
        The maximum minimum SSE to accept. If the minimum SSE is greater
        than this value, an EquilibrationNotDetectedError is raised. The
        default is inf. Units are that of the data.
    ...
    float
        The SSE estimate at the equilibration point.
    """
    # Check that data is valid.
    data = check_data(data, one_dim_allowed=True)
    n_runs, n_samples = data.shape

    # Check that max_lim_var is valid.
    if max_min_lim_var <= 0:
        raise InvalidInputError("max_min_lim_var must be positive.")
    ...
    # Get the SSE timeseries
    sse_vals, sse_times = get_sse_series(
        data=data,
        times=times_valid,
        method=method,
        n_pow=n_pow,
        lugsail_max_discard_frac=0.95,
    )

    # Get the index of the minimum SSE.
    equil_idx = _np.argmin(sse_vals)
    equil_time = sse_times[equil_idx]
    equil_sse = sse_vals[equil_idx]

    # Check that the SSE is less than the maximum SSE.
    if equil_sse > max_min_lim_var:
        raise EquilibrationNotDetectedError(
            f"The minimum SSE of {equil_sse} is greater than the maximum SSE of {max_min_lim_var}."
        )

    # Now compute the limiting variance estimate at this time point.
    equil_data = data[:, equil_idx:]
    equil_sse = sse(equil_data, method=method, n_pow=n_pow)
    ...
    # Return the equilibration index, limiting variance estimate, and SSE.
    return equil_time, equil_sse, n_eff
...
def get_ess_series(
    ...
    ess_vals = _np.zeros(n_samples - n_ignore)
    time_vals = _np.zeros(n_samples - n_ignore)
...
def detect_equilibration_max_ess(
    ...
    ess_vals, ess_times = get_ess_series(
        data=data, times=times, method=method, n_pow=n_pow
    )

    # Get the index of the maximum ESS.
    equil_idx = _np.argmax(ess_vals)
